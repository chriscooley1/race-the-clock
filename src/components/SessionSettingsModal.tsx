...
import { useTheme } from "../context/ThemeContext"; // Ensure the correct path to ThemeContext
import { themes, textColorOptions } from "../themeOptions";

const SessionSettingsModal = ({ onClose, onStart, currentSettings }) => {
        ...
        <div className="modal-content">
          ...
          <div className="settings">
            ...
            <select value={selectedTheme} onChange={e => handleThemeChange(e.target.value)}>
              {themes.map(theme => (
                <option key={theme.className} value={theme.className}>{theme.name}</option>
              ))}
            </select>
          </div>
